...
gc::Variant ruby_r2h(mrb_state *mrb, mrb_value v) {
    switch (v.tt) {
        ...
        case MRB_TT_ARRAY:
        {
            ...
            return gc::Variant(gc::RArray(vs));
        }
        case MRB_TT_STRING:
        {
...
RubyClass *RubyScript::reg_class(struct RClass *rcls, const gc::StringName &name) {
    ...
    if (create) {
        ...
        const gc::Class *hcls = scls->getNativeClass();
...
void RubyScript::reset() {
    ...
    reg_class(mrb->object_class, gc::Object::getClass()->getFullname());
}
...
gc::Variant RubyScript::run(const char *filepath) const {
#ifndef DEBUG
...
gc::ScriptInstance *RubyScript::newBuff(const string &cls_name, gc::Object *target, const gc::Variant **params, int count)const {
    return newBuff(mrb_class_get(mrb, cls_name.c_str()), target, params, count);
}


RubyInstance *RubyScript::newBuff(struct RClass *scls, gc::Object *target, const gc::Variant **params, int count) const {
    mrb_value obj;
    if (count) {
